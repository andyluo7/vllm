...
# Shapes instantiated by ROCm/aiter#4787.
PAGE_SIZE = 128
SUPPORTED_TOPK_BLOCKS = 16
SUPPORTED_INDEX_HEADS = (1, 2)
SUPPORTED_INDEX_HEAD_DIM = 128
SUPPORTED_SCORE_TYPE = "max"
...
def aiter_indexer_unsupported_reason(
    *,
    ...
    if score_type != SUPPORTED_SCORE_TYPE:
        return (
            f"score_type={score_type!r} is not built; "
            f"AITER supports only {SUPPORTED_SCORE_TYPE!r}"
        )
    if topk_blocks != SUPPORTED_TOPK_BLOCKS:
        return (
            f"topk_blocks={topk_blocks} is not built; "
            f"AITER requires {SUPPORTED_TOPK_BLOCKS}"
        )
    if num_index_heads not in SUPPORTED_INDEX_HEADS:
        return (
            f"num_index_heads={num_index_heads} is not built; "
            f"AITER supports only {SUPPORTED_INDEX_HEADS}"
        )
    if index_head_dim != SUPPORTED_INDEX_HEAD_DIM:
        return (
            f"index_head_dim={index_head_dim} is not built; "
            f"AITER requires {SUPPORTED_INDEX_HEAD_DIM}"
        )
    if sparse_block_size != PAGE_SIZE:
        return (
            f"sparse_block_size={sparse_block_size} is not built; "
            f"AITER requires {PAGE_SIZE}"
        )
