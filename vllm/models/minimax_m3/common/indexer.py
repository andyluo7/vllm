...
class MiniMaxM3IndexerImpl(nn.Module):
    ...
    # Set by impls whose top-k also writes the attend's page table.
...
def select_indexer_impl_cls(
    *,
    ...
    if current_platform.is_rocm():
        ...
        aiter_reason = aiter_indexer_unsupported_reason(
        ...
        logger.info_once("MiniMax M3 indexer: AITER unavailable (%s)", aiter_reason)
    ...
    if indexer_kv_dtype != "bf16":
        if aiter_reason is not None:
            raise NotImplementedError(
                f"indexer_kv_dtype={indexer_kv_dtype!r} cannot fall back to "
                f"Triton because AITER is unavailable: {aiter_reason}."
            )
        raise NotImplementedError(
            f"indexer_kv_dtype={indexer_kv_dtype!r} is not supported by the "
            "Triton indexer impl."
